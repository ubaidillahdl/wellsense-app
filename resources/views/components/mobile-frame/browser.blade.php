<div class="sm:flex justify-between h-[60%] items-center px-[2vh]">
    ...
    <div class="w-[70%]" x-data="{
        get shortUrl() {
            let url = window.location.origin.replace(/^https?:\/\//, '');
            return url.length > 20 ? url.substring(0, 12) + '...' : url;
        }
    }">
        <div class="bg-slate-600 mx-[1vh] rounded-full px-[2vh] h-[3vh] text-[1.7vh] flex items-center overflow-hidden">
            <span class=" inline-block align-middle font-semibold text-white" x-text="shortUrl">Loading...</span>
        </div>
    </div>
